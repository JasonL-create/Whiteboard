...
function renderSettings(){
  ...
  rows.innerHTML=`<section class="settings-card"><div class="settings-card-head"><div><h2>Key & Lockbox Setup</h2><p>Manage the office lockbox pool and import key inventory.</p></div></div><div class="settings-item"><div><h3>Lockbox Inventory</h3><p>Add or remove lockboxes available for assignment. Lockboxes currently checked out cannot be removed.</p></div><button id="addConfigureLB" class="primary">+ Add Lockbox</button></div><div class="configure-lb-list">${inventoryRows||'<div class="empty configure-empty">No lockboxes in inventory.</div>'}</div><div class="settings-item import-key-item"><div><h3>Import Key Log</h3><p>Import key tags and property addresses from a CSV file. Existing tag numbers or property addresses are flagged before anything is added.</p></div><button id="importKeyLog" class="primary">Import Key Log</button><input id="keyLogFile" type="file" accept=".csv,text/csv" hidden></div><div id="importPreview"></div></section>`;
