...
function lifecycleDefaults(type){return type==='turn'?{enabled:false,date:'',steps:{notifyTenants:false,deleteActivity:false,utilitiesCanceled:false,setMaFeeZero:false,setReserveZero:false,hideProperty:false}}:{enabled:false,date:'',steps:{createOwner:false,createProperty:false,addTenants:false,transferUtilities:false,createDropbox:false,notifyNewTenants:false,collectKeys:false,collectSd:false,addTenantActivities:false}}}
function lifecycleState(x){const base=lifecycleDefaults(x.type);const src=x.lifecycle||{};return {enabled:!!src.enabled,date:src.date||'',steps:{...base.steps,...(src.steps||{})}}}
function lifecycleHTML(x){if(x.archived)return '';const l=lifecycleState(x);const isTurn=x.type==='turn';const title=isTurn?'Management Closeout':'New Property';const dateLabel=isTurn?'Termination Date':'Management Start Date';const steps=isTurn?[['notifyTenants','Notify Tenants'],['deleteActivity','Delete Activity'],['utilitiesCanceled','Utilities Canceled'],['setMaFeeZero','Set MA Fee to $0.00'],['setReserveZero','Set Reserve to $0.00'],['hideProperty','Hide Property']]:[['createOwner','Create Owner'],['createProperty','Create Property'],['addTenants','Add Tenants'],['transferUtilities','Transfer Utilities'],['createDropbox','Create Dropbox Folder'],['notifyNewTenants','Notify New Tenants'],['collectKeys','Collect Keys'],['collectSd','Collect SD'],['addTenantActivities','Add Tenant Activities']];return `<div class="lifecycle-block ${l.enabled?'is-open':''}"><label class="lifecycle-toggle"><input class="lifecycle-enable" type="checkbox" ${l.enabled?'checked':''}><span>${title}</span></label>${l.enabled?`<div class="lifecycle-body"><label class="lifecycle-date"><span>${dateLabel}</span><input class="lifecycle-date-input" type="date" value="${l.date}"></label><div class="lifecycle-checklist">${steps.map(([key,label])=>`<label><input class="lifecycle-step" data-step="${key}" type="checkbox" ${l.steps[key]?'checked':''}><span>${label}</span></label>`).join('')}</div></div>`:''}</div>`}
