...
function updateProjectTypeCounts(){
 const active=activeData();
 const turning=active.filter(x=>x.type==='turn'&&!!get(x,'Keys Returned')&&!x.completed).length;
 const notices=active.filter(x=>x.type==='turn'&&!get(x,'Keys Returned')&&!x.completed).length;
 const listings=active.filter(x=>x.type==='listing'&&!x.completed).length;
 const labels={
  'type-active':`Turning ${turning}`,
  'type-notice':`Notice ${notices}`,
  'type-listed':`Listing ${listings}`
 };
 document.querySelectorAll('[data-head-sort]').forEach(btn=>{if(labels[btn.dataset.headSort])btn.textContent=labels[btn.dataset.headSort]});
}
function renderRowsOnly(){
 updateProjectTypeCounts();
 ...
 if(sortMode==='default')list.sort((a,b)=>String(b.createdAt||'').localeCompare(String(a.createdAt||''))||(Number(b.id)||0)-(Number(a.id)||0));
 if(sortMode==='pmi-date')prioritySort(x=>x.type==='listing'&&!!get(x,'PMI'),x=>sortDate(x,'PMI'));
 if(sortMode==='notice-date')prioritySort(x=>x.type==='turn'&&!!get(x,'Tenant Gave Notice'),x=>sortDate(x,'Tenant Gave Notice'));
 ...
 if(sortMode==='type-active')prioritySort(x=>x.type==='turn'&&!!get(x,'Keys Returned')&&!x.completed,x=>sortDate(x,'Keys Returned'));
 if(sortMode==='type-listed')prioritySort(x=>x.type==='listing'&&!x.completed,x=>sortDate(x,'Listed'));
 if(sortMode==='type-notice')prioritySort(x=>x.type==='turn'&&!get(x,'Keys Returned')&&!x.completed,x=>sortDate(x,'Tenant Gave Notice'));
 ...
 if(sortMode==='status-mailed')prioritySort(x=>x.type==='turn'&&!!get(x,'Mailed Disposition'),x=>get(x,'Mailed Disposition'));
 if(sortMode==='status-moi')prioritySort(x=>x.type==='turn'&&!!get(x,'MOI')&&!get(x,'Mailed Disposition'),x=>get(x,'MOI'));
 ...
 rows.innerHTML=list.length?list.map(x=>`<section class="row ${x.archived?'archived':''} ${String(openId)===String(x.id)?'open':''}" data-id="${x.id}"><div class="row-main"><div class="type ${x.type==='listing'?listingTypeClass(x):turnTypeClass(x)}">${x.type==='listing'?listingTypeText(x):turnTypeText(x)}${lifecycleState(x).enabled?`<span class="portfolio-dot ${x.type==='listing'?'gain':'loss'}" title="${x.type==='listing'?'New property':'Management closeout'}"></span>`:''}</div><div class="property">${x.address}${recordLocationBadge(x.locationId)}</div><div class="status">${statusHTML(x)}</div>${x.archived?'<div class="archive-pill">ARCHIVED</div>':'<div class="chev">›</div>'}</div>${String(openId)===String(x.id)?detailsHTML(x):''}</section>`).join(''):`<div class="empty">No processes in this view.</div>`;
 bindBoardRows();
}
function bindBoardRows(){rows.querySelectorAll('.row-main').forEach(el=>el.onclick=()=>{const id=el.parentElement.dataset.id;openId=String(openId)===String(id)?null:id;render()});rows.querySelectorAll('.proc-control').forEach(el=>el.onchange=()=>{const x=data.find(y=>String(y.id)===el.closest('.row').dataset.id),p=x.process[+el.dataset.i];p.value=p.kind==='check'?el.checked:el.value;save();render()});rows.querySelectorAll('.proc-note').forEach(el=>{const fit=()=>{el.style.height='auto';el.style.height=Math.min(el.scrollHeight,116)+'px';el.classList.toggle('note-scroll',el.scrollHeight>116)};el.onfocus=()=>{el.classList.add('expanded');fit()};el.oninput=()=>{const x=data.find(y=>String(y.id)===el.closest('.row').dataset.id),p=x.process[+el.dataset.i];p.note=el.value;fit();save()};el.onblur=()=>{el.classList.remove('expanded','note-scroll');el.style.height=''};});rows.querySelectorAll('.notes textarea').forEach(el=>el.oninput=()=>{data.find(y=>String(y.id)===el.closest('.row').dataset.id).notes=el.value;save()});rows.querySelectorAll('.meta-control').forEach(el=>el.onchange=()=>{const x=data.find(y=>String(y.id)===el.closest('.row').dataset.id);x[el.dataset.field]=el.value;save();render()});rows.querySelectorAll('.lifecycle-enable').forEach(el=>el.onchange=()=>{const x=data.find(y=>String(y.id)===el.closest('.row').dataset.id);x.lifecycle=lifecycleState(x);x.lifecycle.enabled=el.checked;save();render()});rows.querySelectorAll('.lifecycle-date-input').forEach(el=>el.onchange=()=>{const x=data.find(y=>String(y.id)===el.closest('.row').dataset.id);x.lifecycle=lifecycleState(x);x.lifecycle.date=el.value;save();render()});rows.querySelectorAll('.lifecycle-step').forEach(el=>el.onchange=()=>{const x=data.find(y=>String(y.id)===el.closest('.row').dataset.id);x.lifecycle=lifecycleState(x);x.lifecycle.steps[el.dataset.step]=el.checked;save();render()});rows.querySelectorAll('.lifecycle-occupancy-check').forEach(el=>el.onchange=()=>{const x=data.find(y=>String(y.id)===el.closest('.row').dataset.id);x.lifecycle=lifecycleState(x);x.lifecycle.occupancy=el.checked?el.dataset.occupancy:'';save();render()});rows.querySelectorAll('.lifecycle-na').forEach(el=>el.onclick=e=>{e.preventDefault();e.stopPropagation();const x=data.find(y=>String(y.id)===el.closest('.row').dataset.id);x.lifecycle=lifecycleState(x);const key=el.dataset.step;x.lifecycle.na[key]=!x.lifecycle.na[key];if(x.lifecycle.na[key])x.lifecycle.steps[key]=false;save();render()});rows.querySelectorAll('.project-complete-check').forEach(el=>el.onchange=()=>{const x=data.find(y=>String(y.id)===el.closest('.row').dataset.id);x.completed=el.checked;if(x.completed)x.keepVisible=true;else x.keepVisible=true;save();render()});rows.querySelectorAll('.keep-visible-check').forEach(el=>el.onchange=()=>{const x=data.find(y=>String(y.id)===el.closest('.row').dataset.id);x.keepVisible=el.checked;if(x.completed&&!x.keepVisible){x.archived=true;x.archivedAt=TODAY;openId=null;}save();render()});rows.querySelectorAll('[data-action]').forEach(b=>b.onclick=e=>{e.stopPropagation();handleAction(b.dataset.action,b.closest('.row').dataset.id)})}
...
function bindShell(){
 ...
 $('#allNav')?.addEventListener('click',()=>{clearSearchForFilter();filter='all';sortMode='default';openId=null;render()});
 ...
 if(view==='keys'){
   $('#allKeysFilter').onclick=()=>{clearSearchForFilter();keyFilter='all';keySort='tag';renderKeys()};
   $('#pageHero').querySelectorAll('[data-key-filter]').forEach(b=>b.onclick=()=>{clearSearchForFilter();keyFilter=b.dataset.keyFilter;renderKeys()});
...
const modal=$('#modal');function showModal(v){writeLocationOverride=null;modal.classList.toggle('hidden',!v);if(v){const wrap=$('#newLocationWrap'),sel=$('#newLocation');if(wrap&&sel){wrap.classList.toggle('hidden',!isAllLocations());sel.innerHTML='<option value="">Choose location…</option>'+turnflowLocations.map(l=>`<option value="${l.id}">${escapeHTML(l.name)}</option>`).join('')}setTimeout(()=>$('#newAddress').focus(),0);updateAddressSuggestions()}else{$('#addressSuggestions')?.classList.add('hidden');$('#addressMatchNote')?.classList.add('hidden')}}const legacyNewProcess=$('#newProcess');if(legacyNewProcess)legacyNewProcess.onclick=()=>showModal(true);$('#closeModal').onclick=$('#cancelModal').onclick=()=>showModal(false);$('#newAddress').addEventListener('input',updateAddressSuggestions);document.querySelectorAll('.project-type-choice').forEach(b=>b.onclick=()=>{document.querySelectorAll('.project-type-choice').forEach(x=>x.classList.remove('selected'));b.classList.add('selected');$('#newType').value=b.dataset.projectType;updateAddressSuggestions()});$('#newForm').onsubmit=async e=>{
 ...
 const x={id:Date.now(),createdAt:new Date().toISOString(),address,type,locationId:creationLocation||'',archived:false,completed:false,keepVisible:true,notes:'',process:type==='turn'?turnProcess():listingProcess()};
 ...
 if(normalizedReady){
   try{
     ...
     const {data:created,error}=await sb.from('projects').insert(row).select('id,created_at').single();
     if(error)throw error;
     x.id=created.id;x.createdAt=created.created_at||x.createdAt;
...
function keyHeaderHTML(){
 const available=keys.filter(k=>!k.address).length;
 const out=keys.filter(k=>!!k.keyOut).length;
...
function renderKeyRowsOnly(){
 const q=(($('#search')?.value)||'').trim().toLowerCase();
 let list=keys.filter(k=>!q||[k.tag,k.address,k.notes,k.lb?.number,k.keyOut?.to,...(k.history||[]).map(h=>h.detail)].filter(Boolean).join(' ').toLowerCase().includes(q));
 if(keyFilter==='available')list=list.filter(k=>!k.address);
 if(keyFilter==='key-out')list=list.filter(k=>!!k.keyOut);
 if(keyFilter==='lb-out')list=list.filter(k=>!!k.lb);
 if(keyFilter==='missing-key')list=list.filter(k=>!!k.keyMissing);
 if(keyFilter==='missing-lb')list=list.filter(k=>!!k.lbMissing);
 list=list.slice().sort((a,b)=>{
   const tagCmp=()=>String(a.tag).localeCompare(String(b.tag),undefined,{numeric:true});
   const addr=x=>String(x.address||'').trim();
   if(keySort==='tag-desc')return -tagCmp();
   if(keySort==='address'||keySort==='address-asc'){
     const aa=addr(a),bb=addr(b);if(!aa&&!bb)return tagCmp();if(!aa)return 1;if(!bb)return -1;return aa.localeCompare(bb,undefined,{numeric:true,sensitivity:'base'})||tagCmp();
   }
   if(keySort==='address-desc'){
   ...
   if(keySort==='lb-assigned'){const aa=a.lb?0:1,bb=b.lb?0:1;if(aa!==bb)return aa-bb;if(a.lb&&b.lb){const an=Number(a.lb.number),bn=Number(b.lb.number);if(Number.isFinite(an)&&Number.isFinite(bn)&&an!==bn)return an-bn;const lc=String(a.lb.number).localeCompare(String(b.lb.number),undefined,{numeric:true,sensitivity:'base'});if(lc)return lc}}
   if(keySort==='missing-lb'){const aa=a.lbMissing?0:1,bb=b.lbMissing?0:1;if(aa!==bb)return aa-bb}
   return tagCmp();
 });
 rows.innerHTML=list.length?list.map(keyRowHTML).join(''):'<div class="empty">No key tags found.</div>';
 rows.querySelectorAll('.key-main').forEach(e=>e.onclick=()=>{keyOpenId=keyOpenId===e.parentElement.dataset.kid?null:e.parentElement.dataset.kid;renderKeys()});bindKeyActions();
}
function renderKeys(){view='keys';renderShell();const head=$('.board-head');head.innerHTML=keyHeaderHTML();head.classList.add('key-board-head');head.style.display='grid';bindKeyHeader();renderKeyRowsOnly()}
function keyRowHTML(k){const keyStatus=k.keyMissing?'MISSING':k.keyOut?`Out to ${k.keyOut.to}`:'';const lbStatus=k.lbMissing&&k.lb?`LB MISSING #${k.lb.number}`:k.lbMissing?'LB MISSING':k.lb?`<span class="lb-assigned-pill"><span>LB #${k.lb.number}</span>${k.lb.outDate?` <span class="lb-pill-date">${short(k.lb.outDate)}</span>`:''}</span>`:'';return `<section class="key-row ${keyOpenId===k.id?'open':''}" data-kid="${k.id}"><div class="key-main"><div><span class="key-sub">TAG</span> <span class="key-tag">#${k.tag}</span></div><div><div class="key-address ${k.address?'':'key-available'}">${k.address||'AVAILABLE'}${recordLocationBadge(k.locationId)}</div></div><div class="key-state ${k.keyMissing?'missing-status':''}">${keyStatus?`<div><span class="key-location-value ${k.keyMissing?'missing-status':''}">${keyStatus}</span>${k.keyOut&&!k.keyMissing?` <span class="key-location-date">${k.keyOut.date}</span>`:''}</div>`:''}</div><div class="key-state key-lb ${k.lbMissing?'missing-status':''}">${lbStatus}</div><div>›</div></div>${keyOpenId===k.id?keyDetailsHTML(k):''}</section>`}
...
function bindKeyActions(){
  rows.querySelectorAll('.key-row').forEach(row=>{
    ...
    const updateButtons=()=>{
      ...
      lbReturn.disabled=!k.lb||!!k.lbMissing||!lbReturnDate.value;
      if(lbSelect)lbSelect.disabled=!!k.lbMissing;
    };
...
function renderReports(){
 ...
 const newProperties=data.filter(x=>x.type==='listing'&&lifecycleState(x).enabled);
 const lostProperties=data.filter(x=>x.type==='turn'&&lifecycleState(x).enabled);
 if(reportTab==='inventory'){
   ...
   const notices=data.filter(x=>x.type==='turn'&&inMonth(get(x,'Tenant Gave Notice'))),turnDone=data.filter(x=>x.type==='turn'&&x.completed&&inMonth(x.completedAt||get(x,'Mailed Disposition'))),rentedMonth=data.filter(x=>x.type==='listing'&&inMonth(get(x,'Signed Lease Received'))),newMonth=newProperties.filter(x=>inMonth(lifecycleState(x).date)),lostMonth=lostProperties.filter(x=>inMonth(lifecycleState(x).date));
   ...
   rows.innerHTML=`<div class="report-page"><div class="report-heading"><h1>${monthName}</h1><p>Current month preview. A frozen month-end snapshot will preserve the final historical report.</p></div><div class="report-metrics">${reportMetric('NOTICES RECEIVED',notices.length,'')}${reportMetric('TURNS COMPLETED',turnDone.length,'')}${reportMetric('TOTAL RENTED',rentedMonth.length,'')}${reportMetric('NEW PROPERTIES',newMonth.length,'')}${reportMetric('LOST PROPERTIES',lostMonth.length,'')}</div><section class="report-detail"><h2>Property Detail</h2>${detail.length?`<div class="monthly-table"><div class="monthly-head"><span>PROPERTY</span><span>TURN</span><span>LISTING</span><span>TOTAL</span></div>${detail.map(r=>`<div class="monthly-row"><strong>${escapeHTML(r.address)}</strong><span>${r.turn==null?'—':r.turn+' days'}</span><span>${r.listing} days</span><span>${r.total==null?'—':r.total+' days'}</span></div>`).join('')}</div>`:'<div class="empty">No properties rented this month yet.</div>'}</section></div>`;
 } else {
...
async function loadUserAccessAdmin(){
 ...
 host.innerHTML=users.map(u=>{const a=u.locations[0];const revoked=u.request_status==='revoked'&&!a;const level=a?.access_level||'staff';const loc=a?.location_id||'';const companyWide=['management','admin'].includes(level);const isSelf=String(u.user_id)===String(cloudUser.id);const locationControl=companyWide?`<select class="user-location-select company-wide" data-user="${u.user_id}" disabled><option value="all" selected>All Locations</option></select>`:`<select class="user-location-select" data-user="${u.user_id}"><option value="">Choose location…</option>${turnflowLocations.map(l=>`<option value="${l.id}" ${String(loc)===String(l.id)?'selected':''}>${escapeHTML(l.name)}</option>`).join('')}</select>`;return `<div class="user-access-row ${!a?'pending-user':''}"><div class="user-access-person"><input class="user-name-input" data-user="${u.user_id}" value="${escapeHTML(u.display_name||'')}" placeholder="Name" aria-label="User name"><span>${escapeHTML(u.email)}</span>${revoked?'<em>Revoked</em>':(!a?'<em>Pending access</em>':'')}</div>${locationControl}<select class="user-role-select" data-user="${u.user_id}"><option value="staff" ${level==='staff'?'selected':''}>Staff</option><option value="management" ${level==='management'?'selected':''}>Management</option><option value="admin" ${level==='admin'?'selected':''}>Admin</option></select><div class="user-access-actions"><button class="primary save-user-access" data-user="${u.user_id}" ${a?'disabled':''}>${a?'Save':(revoked?'Restore Access':'Approve Access')}</button>${a&&!isSelf?`<button class="action-secondary revoke-user-access" data-user="${u.user_id}">Revoke</button>`:''}</div></div>`}).join('')||'<div class="empty configure-empty">No users found.</div>';
 host.querySelectorAll('.save-user-access').forEach(b=>b.onclick=async()=>{const id=b.dataset.user,level=host.querySelector(`.user-role-select[data-user="${id}"]`)?.value,name=(host.querySelector(`.user-name-input[data-user="${id}"]`)?.value||'').trim();let loc=host.querySelector(`.user-location-select[data-user="${id}"]`)?.value;if(!name){await tfAlert('Name Required','Enter a name for this user.');return}if(['management','admin'].includes(level)){const user=users.find(x=>String(x.user_id)===String(id));loc=user?.locations?.[0]?.location_id||turnflowLocations[0]?.id||''}if(!loc){await tfAlert('Location Required','Choose a location before approving access.');return}b.disabled=true;const nameResult=await sb.rpc('update_turnflow_user_display_name',{p_organization_id:cloudOrgId,p_user_id:id,p_display_name:name});if(nameResult.error){b.disabled=false;await tfAlert('Could Not Save Name',escapeHTML(nameResult.error.message));return}const {error}=await sb.rpc('set_turnflow_user_access',{p_organization_id:cloudOrgId,p_user_id:id,p_location_id:loc,p_access_level:level});b.disabled=false;if(error){await tfAlert('Could Not Save Access',escapeHTML(error.message));return}syncToast('User saved');await refreshAdminPendingCount();renderShell();await loadUserAccessAdmin()});
 const updateUserAccessSaveState=id=>{const user=users.find(x=>String(x.user_id)===String(id));const btn=host.querySelector(`.save-user-access[data-user="${id}"]`);if(!user||!btn)return;const a=user.locations[0];if(!a){btn.disabled=false;return}const role=host.querySelector(`.user-role-select[data-user="${id}"]`)?.value||'staff';const savedRole=a.access_level||'staff';const name=(host.querySelector(`.user-name-input[data-user="${id}"]`)?.value||'').trim();const savedName=String(user.display_name||'').trim();let changed=role!==savedRole||name!==savedName;if(!['management','admin'].includes(role)){const loc=host.querySelector(`.user-location-select[data-user="${id}"]`)?.value||'';changed=changed||String(loc)!==String(a.location_id||'')}btn.disabled=!changed};
 host.querySelectorAll('.user-role-select').forEach(sel=>sel.onchange=()=>{const id=sel.dataset.user;const locSel=host.querySelector(`.user-location-select[data-user="${id}"]`);if(!locSel)return;const companyWide=['management','admin'].includes(sel.value);if(companyWide){locSel.innerHTML='<option value="all" selected>All Locations</option>';locSel.disabled=true;locSel.classList.add('company-wide')}else{const user=users.find(x=>String(x.user_id)===String(id));const saved=user?.locations?.[0]?.location_id||'';locSel.innerHTML='<option value="">Choose location…</option>'+turnflowLocations.map(l=>`<option value="${l.id}" ${String(saved)===String(l.id)?'selected':''}>${escapeHTML(l.name)}</option>`).join('');locSel.disabled=false;locSel.classList.remove('company-wide')}updateUserAccessSaveState(id)});
 host.querySelectorAll('.user-location-select').forEach(sel=>sel.onchange=()=>updateUserAccessSaveState(sel.dataset.user));
 host.querySelectorAll('.user-name-input').forEach(inp=>inp.oninput=()=>updateUserAccessSaveState(inp.dataset.user));
...
function rowToProject(r,propertyAddress){
  ...
  return {
    id:r.id,createdAt:r.created_at||'',address:propertyAddress,type:r.project_type,locationId:r.location_id||'',
    ...
    lifecycle:{...lifecycleDefaults(r.project_type),...(w.lifecycle||{}),steps:{...lifecycleDefaults(r.project_type).steps,...((w.lifecycle||{}).steps||{})}},
