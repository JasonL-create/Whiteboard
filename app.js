...
function renderShell(){
 ...
 $('#masterNav').innerHTML=`<div class="master-left"><button id="projectsNav" class="nav-btn ${view==='board'&&filter!=='archived'?'selected':''}"><img class="nav-icon-img" src="icon-projects.png" alt="">PROJECTS</button><button id="keysNav" class="nav-btn ${view==='keys'?'selected':''}"><img class="nav-icon-img" src="icon-keys.png" alt="">KEYS</button><button id="archiveNav" class="nav-btn ${view==='board'&&filter==='archived'?'selected':''}"><img class="nav-icon-img" src="icon-archive.png" alt="">ARCHIVE <b>${arch}</b></button><button class="nav-btn future-nav" disabled><img class="nav-icon-img" src="icon-reports.png" alt="">REPORTS</button><button id="settingsNav" class="nav-btn ${view==='settings'?'selected':''}"><img class="nav-icon-img" src="icon-configure.png" alt="">CONFIGURE</button></div><div class="master-search"><div class="search-wrap"><input id="shellSearch" class="shell-search" type="search" placeholder="Search" value="${q}"></div></div>`;

 const hero=$('#pageHero');
 if(view==='keys'){
   const out=keys.filter(k=>k.keyOut||k.keyMissing).length,lbOut=keys.filter(k=>k.lb).length;
   hero.innerHTML=`<div class="page-title-inline">KEYS</div><div class="context-left"><button id="allKeysFilter" class="filter-btn ${keyFilter==='all'?'active':''}">ALL <b>${keys.length}</b></button><button id="lbOutFilter" class="filter-btn ${keyFilter==='lb'?'active':''}">LB OUT <b>${lbOut}</b></button><button id="keysOutFilter" class="filter-btn ${keyFilter==='keys'?'active':''}">KEYS OUT <b>${out}</b></button><div class="nav-dropdown"><button class="filter-btn ${keySort!=='tag'?'active':''}">SORT BY</button><div class="dropdown-panel"><button data-key-sort="tag">TAG NUMBER</button><button data-key-sort="address">PROPERTY ADDRESS</button><button data-key-sort="available">AVAILABLE FIRST</button><button data-key-sort="assigned">ASSIGNED FIRST</button></div></div></div><div class="page-actions"><button id="addKeyTagHero" class="hero-add"><span>＋</span> Add Key Tag</button></div>`;
...
function bindShell(){
 ...
 $('#pageHero').querySelectorAll('[data-f]').forEach(b=>b.onclick=()=>{view='board';filter=b.dataset.f;openId=null;render()});
...
function rowToProject(r,propertyAddress){
  ...
  return {
    ...
    process:r.project_type==='listing'?reconcileListingProcess(Array.isArray(w.process)?w.process:[]):(Array.isArray(w.process)?w.process:turnProcess())
  };
}
